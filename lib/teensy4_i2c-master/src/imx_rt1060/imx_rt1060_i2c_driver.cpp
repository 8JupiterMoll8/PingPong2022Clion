...
bool IMX_RT1060_I2CMaster::start(uint8_t address, uint32_t direction) {
    ...
    // Send a IDLE to the slave at 'address'
...
//    log_slave_status_register(ssr);
    ...
    if (ssr & (LPI2C_SSR_RSF | LPI2C_SSR_SDF)) {
        // Detected Repeated IDLE or STOP
    ...
    if (ssr & LPI2C_SSR_RDF) {
        ...
        if (srdr & LPI2C_SRDR_SOF) {
            // Start of Frame (The first byte since a (repeated) IDLE or STOP condition)
            _error = I2CError::ok;
...
// Called from within the ISR when we receive a Repeated IDLE or STOP
